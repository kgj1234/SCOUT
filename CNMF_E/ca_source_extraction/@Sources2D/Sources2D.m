    ...
    methods
        ...
        function obj = Sources2D(varargin)
            ...
            obj.P =struct('mat_file', [], 'mat_data', [], 'indicator', '', 'k_options', 0, ...
                ...
                'kernel_pars',[], 'k_ids', 0,'add_noise',false);
            if nargin>0
        ...
        [center, Cn, pnr] = initComponents_endoscope(obj, Y, K, patch_sz, debug_on, save_avi,initial_centers);
        ...
        [merged_ROIs, newIDs] = quickMerge(obj, merge_thr,gSiz);
        ...
        function ids = remove_false_positives(obj, show_delete,link)
            if ~exist('link','var')
                link=false;
            end
            ...
            if link
                tmp1=obj.copy();
                tmp2=obj.copy();
                tmp1.C=tmp1.C(:,1:ceil(size(obj.C,2)/2));
                tmp1.S=tmp1.S(:,1:ceil(size(obj.C,2)/2));
                tmp1.C_raw=tmp1.C_raw(:,1:ceil(size(obj.C,2)/2));
                tmp2.C=tmp2.C(:,ceil(size(obj.C,2)/2):end);
                tmp2.S=tmp2.S(:,ceil(size(obj.C,2)/2):end);
                tmp2.C_raw=tmp2.C_raw(:,ceil(size(obj.C,2)/2):end);
            end
        end
        ...
        function delete(obj, ind)
            ...
            try obj.SNR(ind)=[]; catch; end
              ...
              try   obj.Coor(ind)=[]; catch; end
               try  obj.Df(ind)=[]; catch; end
             try    obj.C_df(ind,:)=[]; catch; end
            try    obj.centroid(ind,:)=[]; catch; end
           try     obj.trace(ind,:)=[]; catch; end
           try obj.corr_scores(ind,:)=[]; catch; end
           try obj.corr_prob(ind,:)=[]; catch; end
           try obj.overlap_scores(ind,:)=[]; catch; end
           try obj.overlap_prob(ind,:)=[]; catch; end
           try obj.KL_scores(ind,:)=[]; catch; end
           try obj.KL_prob(ind,:)=[]; catch; end
           try obj.dist_scores(ind,:)=[]; catch; end
           try obj.dist_prob(ind,:)=[]; catch; end
           try obj.existant_indices(ind,:)=[]; catch; end
           try obj.probabilities(ind,:)=[]; catch; end
           try obj.normalized_probabilities(ind,:)=[]; catch; end
           try obj.corr_prob_normalized(ind,:)=[]; catch; end
           try obj.KL_prob_normalized(ind,:)=[]; catch; end

function Yest  = local_backgroundnonpar(Y, ssub, rr, ACTIVE_PX, sn, thresh, p_cutoff)
...
Y = reshape(Y, d1s*d2s, []);
Yest = cell(1,length(ind_px));
weights = cell(d1s, d2s);
 
parfor m=1:length(ind_px)
    px = ind_px(m);
...
%     J = length(ind_nhood);
    ...
    Yest{m} = w'*Y(ind_nhood, :);
    weights{m} = [ind_nhood; w'];
end
Yest1=zeros(size(Y));
weights1=cell(size(weights));
for m=1:length(ind_px)
    px=ind_px(m);
    Yest1(px,:)=Yest{m};
    weights1{px}=weights{m};
end
Yest=Yest1;
weights=weights1;
clear Yest1;

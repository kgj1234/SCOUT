...
%   Jinghao Lu, 07/01/2018
    ...
    h = tic;
    if Params.bg_remove_ispara
        if isempty(gcp('nocreate'))
            parpool(floor(3/4*feature('numCores')));
        end
    end
    overwrite_flag=true;
    %% batch preparation %%
    ...
    nsize = pixh * pixw * nf * stype * floor(feature('numCores')*.75) + pixh * pixw * 8 * 8 * 2; %%% size of stype * number of parallel cores + 8 convoluton * 8 for double * 2 variables %%%

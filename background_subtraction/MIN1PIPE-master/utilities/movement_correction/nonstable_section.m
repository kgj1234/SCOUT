...
%   Jinghao Lu, 02/02/2018
    ...
    if ~isempty(bstt)
        ...
        for i = 1: nbatch
            ...
            regpara_orig=cell(1,idbatch(i+1)-idbatch(i));
            for ii = idbatch(i) + 1: idbatch(i + 1)
                regpara{ii - idbatch(i)} = m.reg(1: pixh, 1: pixw, max(1, bstt(ii) - 1): min(nf, bstp(ii)));
                regpara_orig{ii-idbatch(i)}=m.orig(1:pixh,1:pixw,max(1,bstt(ii)-1):min(nf,bstp(ii)));
            end
            ...
            if bstt(1) == 1
                if i == 1
                    ...
                    regpara_orig{1}=cat(3,m.orig(1:pixh,1:pixw,sttn(1)),regpara_orig{1});
                end
            end
            ...
            parfor ii = idbatch(i) + 1: idbatch(i + 1)
                ...
                regcur_orig=double(regpara_orig{ii-stof});
                %%%% round 1 logdemons %%%%
                [regcur,~,~,~,PNs,p0] = lk_logdemons_unit(regcur, se, pixs, scl, sigma_x, sigma_f, sigma_d, flag, maskc);
                nn = size(regcur,3);
                for j = 1: nn - 1
                    regcur_orig(:, :, j + 1) = lk2_warp(regcur_orig(:, :, j + 1), PNs(:, j), p0);
                ...
                for j = 1: length(idc)
                    ...
                    if sct1 > sct2
                        ...
                        regcur_orig(:,:,idc(j)+1)=regcur_orig(:,:,idc(j));
                    else
                        regcur(:, :, idc(j)) = regcur(:, :, idc(j) + 1);
                        regcur_orig(:,:,idc(j)+1)=regcur_orig(:,:,idc(j)+1);
                    end
                end
                ...
                regpara_orig{ii-stof}=regcur_orig;
            ...
            for ii = idbatch(i) + 1: idbatch(i + 1)
                ...
                m.orig(1:pixh,1:pixw,bstt(ii):bstp(ii))=regpara_orig{ii-idbatch(i)}(:,:,2:end);
            end
        end
    end
end

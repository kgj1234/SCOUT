...
%   Jinghao Lu, 02/02/2018
    ...
    for i = 1: nbatch
        ...
        regtpara_orig=cell(1,idbatch(i+1)-idbatch(i));
        for j = idbatch(i) + 1: idbatch(i + 1)
            regtpara{j - idbatch(i)} = m.reg(1: pixh, 1: pixw, stt(j): stp(j));
            regtpara_orig{j-idbatch(i)}=m.orig(1:pixh,1:pixw,stt(j):stp(j));
        end
        ...
        parfor ii = 1: length(regtpara)
            ...
            regtcur_orig=logdemons_warp_layers(squeeze(mat2cell(double(regtpara_orig{ii}), pixh, pixw, ones(1, ncur))), xfcur, ldcur)
            regtcur = reshape(cell2mat(regtcur(:)'), pixh, pixw, ncur);
            regtcur_orig=reshape(cell2mat(regtcur_orig(:)'),pixh,pixw,ncur);
            regtpara{ii} = regtcur;
            regtpara_orig{ii}=regtcur_orig;
        ...
        for j = idbatch(i) + 1: idbatch(i + 1)
            ...
            m.orig(1:pixh,1:pixw,stt(j):stp(j))=regtpara_orig{j-idbatch(i)};
        end
    end
end
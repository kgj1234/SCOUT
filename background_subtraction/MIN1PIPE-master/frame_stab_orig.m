...
%   Jinghao Lu, 10/28/2018
    ...
    for i = 1: nbatch
        tmp = m.orig(1: pixh, 1: pixw, idbatch(i) + 1: idbatch(i + 1));
        tmp = convn(tmp, hcol, 'same');
        tmp = convn(tmp, hrow, 'same');
        m.orig(1: pixh, 1: pixw, idbatch(i) + 1: idbatch(i + 1)) = tmp;
    end
    ...
    for i = 1: nbatch
        tmp = m.orig(idbatch(i) + 1: idbatch(i + 1), 1: pixw, 1: nf);
        tmp = convn(tmp, hslc, 'same');
        m.orig(idbatch(i) + 1: idbatch(i + 1), 1: pixw, 1: nf) = tmp;
    end
end

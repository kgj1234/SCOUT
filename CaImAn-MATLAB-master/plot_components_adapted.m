%%
function plot_components_adapted(Y,A,C,b,f,Cn,options,filepatht)
...
set(gcf,'Position',2*[50,50,960,480]);
...
if ~isempty(filepatht)
    filepathtt=[filepatht,'\','cell_information'];
else
    filepathtt=['cell_information'];
end
    mkdir(filepathtt);
    for i=1:size(C,1)
...
%             pause(0.05);
%         end
       saveas(gcf,[filepathtt,'\','cell#',num2str(i),'.fig']);
       saveas(gcf,[filepathtt,'\','cell#',num2str(i),'.tif']);
       saveas(gcf,[filepathtt,'\','cell#',num2str(i),'.eps']);
    end

...
for i=1:size(Aor,2)
    temp=zeros(d1,d2);
    temp(Aor(:,i)>0)=1;
    stats=regionprops(temp,'MajorAxisLength');
    MAL(i)=stats.MajorAxisLength;
end

if display_numbers
    lbl = strtrim(cellstr(num2str((1:size(Aor,2))')));
    text(round(cm(1:max_number,2))+2*MAL'/3,round(cm(1:max_number,1)),lbl(1:max_number),'color',[0,0,0],'fontsize',12,'fontname',fontname,'fontweight','bold');

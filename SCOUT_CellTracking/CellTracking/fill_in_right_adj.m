function [aligned_neurons,aligned_probabilities]=fill_in_right_adj(aligned_neurons,aligned_probabilities,pair_aligned,probabilities,session_ind,neuron_ind,min_prob)
try       
ind=find(pair_aligned{session_ind}(:,2)==aligned_neurons(neuron_ind,session_ind+1));
    ...
    ind=[];
       ...
       try
       avail=pair_aligned{session_ind}(ind,1);
       catch
           ind=[];
       end
           probabilities_agg=probabilities{session_ind,session_ind+1}(ind);
            ...
            for q=1:length(ind)
                ...
                aligned_probabilities(end,session_ind)=probabilities_agg(q);

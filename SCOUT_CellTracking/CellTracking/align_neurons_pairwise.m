function [aligned_neurons,aligned_probabilities]=align_neurons_pairwise(pair_aligned,probabilities,size_vec)
...
for i=1:(length(probabilities)-1)
   used=[];
    for j=1:size(aligned_neurons{i}(:,end),1)
        ...
        try
            pair_ind=find(pair_aligned{i}(:,end-1)==aligned_neurons{i}(j,end));
        catch
            pair_ind=[];
        end
        if ~isempty(pair_ind)
      
        curr_aligned{j}=[repmat(aligned_neurons{i}(j,:),length(pair_ind),1),pair_aligned{i}(pair_ind,2)];
    ...
    if isempty(pair_aligned{i})
        unused=1:size_vec(i+1);
    else
        unused=setdiff(1:size_vec(i+1),pair_aligned{i}(used,end));
    end

...
for i=1:length(neurons)-1
    mid_value(i)=ceil(sum(global_extraction_parameters.batch_sizes{i}...
        (end-global_extraction_parameters.overlap_per_batch...
        +1:end))/2);
    odd(i)=mod(sum(global_extraction_parameters.batch_sizes{i}...
        (end-global_extraction_parameters.overlap_per_batch...
        +1:end)),2);
...
try
    ...
    temp_neurons{1}.C_df=neurons{1}.C_df(:,1:end-mid_value(1));
end

for i=2:length(neurons)-1
    if odd(i-1)
        mid_value(i-1)=mid_value(i-1)-1;
    end
    temp_neurons{i}=neurons{i}.copy();
    ...
    try
        ...
        temp_neurons{i}.C_df=neurons{i}.C_df(:,mid_value(i-1)+1:end-mid_value(i));
        
    end
end
if odd(end)
    mid_value(end)=mid_value(end)-1;
end
...
try
    ...
    temp_neurons{end}.C_df=neurons{end}.C_df(:,mid_value(end)+1:end);
end
else
    for k=1:length(neurons)-1
        batch_size=global_extraction_parameters.batch_sizes{k};
        sess_length=sum(batch_size(1:length(batch_size)-global_extraction_parameters.overlap_per_batch));
        temp_neurons{k}=neurons{k}.copy();
        temp_neurons{k}.C=neurons{k}.C(:,1:sess_length);
        temp_neurons{k}.S=neurons{k}.S(:,1:sess_length);
        try
            temp_neurons{k}.C_raw=neurons{k}.C_raw(:,1:sess_length);
            temp_neurons{k}.C_df=neurons{k}.C_df(:,1:sess_length);
        end
    end

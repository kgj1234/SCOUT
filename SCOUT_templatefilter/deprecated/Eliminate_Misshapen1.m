function [neuron,KL]=Eliminate_Misshapen1(neuron,KL_thresh,data_shape,constraint_type,width_crit,gSizMax,gSizMin,filter)
...
for i=1:length(A)
    ...
    thresh_indices=A1<.7*max(max(A1));
    A1(thresh_indices)=0;
...
%     
%     curr_max=max(max(A1));
%     A1_temp(A1_temp<.6*curr_max)=0;
%     A1_temp=imgaussfilt(A1_temp);
%     gauss_A=zeros(data_shape(1),data_shape(2));
%     [centroid,cov]=calculateCentroid_and_Covariance(A1_temp,data_shape(1),data_shape(2));
%     gauss_A=mvnpdf([X1,Y1],centroid,cov);
%     [KL1(i),mass1(i)]=KLDiv(gauss_A,A1_temp);
%     KL(i)=max(KL(i),KL1(i));
%     
%     
    bw=A1>0;

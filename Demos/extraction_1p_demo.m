...
%Remove 2p extraction code from matlab path
clear all
clc
...
% extraction_options: structure with following possible fields. 
    ...
    %merge_thr (3 element vector) indicate threshold for merging. Order
    %(spatial correlation,signal correlation,spike correlation)
    %dmin (2 element vector) neighbor merging (min_pixel,
        %decay_rate_difference)
...
extraction_options.min_corr=.3; %(Unrestricted initialization, set to .8 for Restricted initialization)
extraction_options.corr_noise=false;
extraction_options.merge_thr=[.95,.95,-1];
extraction_options.dmin=[.5,15]; %(first element should generally be larger for in vivo data)

%The following code can also be run using the following function
%full_demo_endoscope(filename,extraction_options)
